...
/**
 * Hero visual: full-section background — the same premium blue 3D layered-wave
 * render (CGI + real-time 3D language) in a wide 16:9 crop, so `cover` fills
 * desktop/laptop viewports with minimal cropping and no upscaling.
 * Served at 2400px wide (q80, auto=format → WebP/AVIF): covers 1920px screens
 * with headroom, so the backdrop stays sharp, never pixelated or stretched.
 * Abstract waves have no single focal subject, so `cover` crops safely on
 * tablet/mobile; object-position is biased slightly high to keep the wave
 * ridges in frame. Decorative — the headline carries the meaning.
 */
export const HERO_MEDIA = {
  src: unsplash('1634017839464-5c339ebe3cb4', 2400, 1350, 80),
  w: 2400,
  h: 1350,
  position: '50% 38%',
  alt: '',
